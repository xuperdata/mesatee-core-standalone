...
use sgx_types::{
    sgx_attributes_t, sgx_hmac_256bit_key_t, sgx_sealed_data_t, uint16_t, SGX_FLAGS_KSS,
    SGX_KEYPOLICY_CONFIGID, SGX_KEYPOLICY_ISVEXTPRODID, SGX_KEYPOLICY_ISVFAMILYID,
    SGX_KEYPOLICY_MRSIGNER, TSEAL_DEFAULT_FLAGSMASK, TSEAL_DEFAULT_MISCMASK,
};
...
static HMAC_1ST_DERIVATION_LABEL: &'static str = "XUPERDATA-SGX-LEDGER-SEALING-KEY-DERIVATION-KEY";
static HMAC_2ND_DERIVATION_LABEL: &'static str = "XUPERDATA-SGX-LEDGER-RECORD-SEALING-KEY";
...
pub fn encryptex(
    ...
    if ciphertext.len() < plaintext.len() + AES_256_GCM.tag_len() {
    ...
    let mut in_out = vec![0; plaintext.len() + AES_256_GCM.tag_len()];
...
pub fn decryptex(
    ...
    if plaintext.len() < ciphertext.len() - AES_256_GCM.tag_len() {
        return Err(Error::from(ErrorKind::InvalidInputError));
    }
    ...
    let mut in_out = ciphertext.to_owned();
    ...
    plaintext.copy_from_slice(&decrypted_data[0..decrypted_data.len() - AES_256_GCM.tag_len()]);
    Ok(())
}
...
impl KeyManagerment {
    ...
    pub fn seal_keys_for_serializable(
        ...
        if (report.body.attributes.flags & SGX_FLAGS_KSS) != 0 {
            key_policy |= KEY_POLICY_KSS;
        }
        let encoded_data_vec = serde_json::to_vec(&self.kds_map)?;
        let encoded_data_slice = encoded_data_vec.as_slice();
        let additional_text: [u8; 8] = [0; 8];
        ...
        let seal_data = match result {
            Ok(x) => x,
            Err(err) => {
                error!("seal_data_ex {:?}", err);
                return Err(Error::from(ErrorKind::InvalidInputError));
            }
        };
        let opt = to_sealed_log_for_slice(&seal_data, sealed_log, seal_log_size as u32);
        if opt.is_none() {
            return Err(Error::from(ErrorKind::InvalidInputError));
        }
        Ok(())
    ...
    pub fn unseal_keys(&mut self, cipher: *mut u8, cipher_size: usize) -> Result<()> {
        let sealed_data = match from_sealed_log_for_slice::<u8>(cipher, cipher_size as u32) {
            Some(x) => x,
            None => {
                return Err(Error::from(ErrorKind::InvalidInputError));
            }
        ...
        let unseal_data = match sealed_data.unseal_data() {
            Ok(x) => x,
            Err(e) => {
                error!("sealed_data.unseal_data erro: {:?}", e);
                return Err(Error::from(ErrorKind::InvalidInputError));
            }
        };
        let encoded_slice = unseal_data.get_decrypt_txt();
        let data = match serde_json::from_slice(encoded_slice) {
            Ok(x) => {
                self.kds_map = x;
            }
            Err(err) => {
                error!("serde_json::from slice {:?}", err);
                return Err(Error::from(ErrorKind::InvalidInputError));
            }
        };
        debug!("{:?}", data);
